...
Still on the resource configuration, click the response type for status `200` and choose **Create New Type**:
...
Click **Save** on the resource. The configuration should now show `EmailGeneratePayload` for the request and `EmailGenerateResponse` for the `200` response.
...
2. Leave **Result Type** at the default.
3. Click **Save**.
...
The prompt has two natural parts:

| | |
|---|---|
| **Role** | *"You are an email writing assistant."* |
| **Task** | *"Write a short email…suggest two possible time slots…polite, professional tone."* |

> **Why no "return JSON" instruction?** The next field handles that for you.
...
The service responds with `201 Created` and a JSON body matching `EmailGenerateResponse`:
...
- **[Writing Effective Prompts](/docs/genai/key-concepts/writing-effective-prompts)** — interpolate the request fields (`${recipientName}`, `${timeSlots}`) directly into the prompt instead of asking the LLM to invent them.
