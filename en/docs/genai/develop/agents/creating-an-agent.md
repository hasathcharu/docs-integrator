...
| **+ Create New Model Provider** | Link to add a brand-new provider connection without leaving the panel — opens the Select Model Provider picker. See [Adding a Model Provider](/docs/genai/develop/direct-llm/overview#adding-a-model-provider) for the full picker. |
